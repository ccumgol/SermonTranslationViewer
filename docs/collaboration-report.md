...
### 현재 진행 중
```
- [시작 2026-07-27 / Agent C] 후속 개선 4건 (사용자 선택, 각 독립 커밋)
  1) stop.sh 종료 스크립트 (작음·안전) — 고아 서버 원클릭 종료
  2) 무발화 자동 방송 종료 (IDLE_STOP_MIN, 기본 off·경고 후 종료) — 온라인 비용 방지
  ...
  진행: 1) stop.sh ✅ / 2) 무발화 자동종료 ✅ / 3) 입력 게인 ✅(1~8배, 98 passed) / 4) WS 재연결 ⏳ 다음
```
