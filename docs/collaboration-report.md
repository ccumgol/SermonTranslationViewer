...
4. **커밋 정책**: 이 프로젝트에서는 **사용자(소유자)가 커밋합니다.** Agent 는 코드·문서만
   수정하고 커밋/푸시하지 않습니다. (요청이 있을 때만 예외)
...
| 참여자 | 담당·기여 | 최근 작업일 |
|---|---|---|
| **사용자 (소유자)** | 요구사항 정의, 실기기 테스트, **커밋/푸시 권한**, 결제·API 키 관리 | 계속 |
...
| 테스트 | **31개 통과** — 단위 20(순수 로직) + WS·인증 통합 11(보안 회귀 방지) |
| 문서 | README·setup-guide·troubleshooting·handover·analysis (한/영) |
| 보안 | **1단계 하드닝 완료**(C-1·C-2·H-1). H-2~H-4, M/L 은 미착수 |
...
4. 토큰 파일은 `.gitignore` 에 `data/runtime/` 추가로 제외.

### 미커밋 상태 (사용자가 커밋 예정)
```
M .env.example   M .gitignore   M README.md
M server/config.py   M server/ws_server.py   M start.sh
?? docs/analysis-report.md, .en.md   (Agent A 산출물 — 손대지 않음)
?? data/runtime/   (git 제외 대상)
```
...
| H-3 | 운영 명령 rate limit(쿨다운) | 🔴 미착수 | — | `set_backend`/`set_languages` 연타 방지 |
| H-4 | WS 메시지 크기·연결 수 상한 | 🔴 미착수 | — | 원고 길이 상한 포함 |
| M-1 | init 내부정보를 인증 후 전송 | 🔴 미착수 | — | 장치명 등 |
| M-2 | `set_style` 화이트리스트 검증 | 🔴 미착수 | — | Pydantic |
...
| 높음 | 모바일 언어 선택 유지(localStorage) | 🔴 미착수 | — |
...
2. **H-3/H-4** — 명령 쿨다운, WS 크기·연결 상한, 원고 길이 상한. 동작 변경 폭이 있으니
   사용자와 기준값(쿨다운 초, 최대 길이) 합의 후 진행. **테스트 기반이 갖춰졌으니 지금이 적기.**
