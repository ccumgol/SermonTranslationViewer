...
### 현재 진행 중
```
(진행 중 작업 없음 — 운영자 대시보드 오른쪽 열 모바일+원고 세로 배치 완료. 3.18 참고)
```
...
- **후속 재배치(2026-08-13, 요청)**: 오디오~원고 4카드를 `.col`(flex column) 2개로 묶어
  왼쪽 열=오디오 입력 소스+번역 언어, 오른쪽 열=모바일 자막주소+설교 원고(빈틈 없이 이어서).
  auto-placement 로 2열, 1920x1080 에서 두 열 top 정렬·`overflow 0` 재확인.
