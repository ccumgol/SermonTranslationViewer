...
송출 창의 타이틀 바·`localhost:8000` 주소줄을 없애는 3가지 방법:

| 방법 | 결과 | 창 이동 |
|---|---|---|
| **전체화면** (화면 클릭 / `F` / ⛶) | **완전한 검은 화면**, 모든 UI 제거 | 전체화면은 이동 불가 → **먼저 프로젝터 화면으로 옮긴 뒤** 전체화면 |
| **앱으로 설치**(PWA) | 얇은 **타이틀 바 O, 주소줄 X**, 이동·크기조절 자유 | 일반 창처럼 자유 이동 |
| **OBS 브라우저 소스** | UI 전혀 없음(영상 위 자막만) | OBS 안에서 배치 |

- `localhost:8000` **주소줄은 Chrome이 팝업에 강제**하므로 일반 창에선 못 없앱니다.
  → **전체화면**(완전 제거) 또는 **PWA 설치**(주소줄만 제거)를 쓰세요.
- **PWA 설치법**: 송출 화면에서 Chrome 주소창 오른쪽 **설치 아이콘**(또는 메뉴 →
  "앱 설치") 클릭 → 이후 앱 창(타이틀 바만, 주소줄 없음, 이동 자유)으로 열립니다.
- **실전 권장**: 송출 창을 프로젝터 화면으로 옮긴 뒤 **화면을 클릭해 전체화면** → 순수 검정.
