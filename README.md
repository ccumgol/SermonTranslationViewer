...
2. 운영자 화면의 **📱 모바일 주소 / QR 추출** 버튼 클릭 → 접속 주소와 **QR 코드**가 표시됨
3. 폰으로 **QR 스캔**(또는 주소 입력) → 자막 페이지 접속
4. 상단에서 언어 선택 → 그 언어 자막만 표시 (읽기 전용)

> IP 검출·QR 생성 모두 **로컬에서 처리**되어 인터넷 없이 동작합니다.
> 수동 확인도 가능: `ipconfig getifaddr en0` → `http://<IP>:8000/m`
...
운영 명령(언어 변경, 방송 종료 등)은 **항상 토큰 인증**이 필요합니다.

- **토큰 자동 생성**: LAN 에 열려 있는데(`WS_HOST=0.0.0.0`) `OPERATOR_TOKEN` 이 없으면
  기동 시 임시 토큰이 생성되고 콘솔에 접속 URL 이 출력됩니다.
  `./start.sh` 는 그 URL(토큰 포함)로 브라우저를 엽니다. → **"인증 없음" 상태가 없습니다.**
- 매번 같은 주소를 쓰려면 `.env` 에 `OPERATOR_TOKEN=<긴 무작위 문자열>` 을 지정하세요.
- **Origin 검증**: 이 서버가 서빙하는 화면(같은 호스트)과 localhost 에서 온 WebSocket 만
  허용합니다. 외부 웹페이지가 몰래 붙어 명령하거나 전사를 도청하는 CSWSH 를 차단합니다.
  (리버스 프록시 등 다른 도메인을 쓰면 `ALLOWED_ORIGINS` 에 추가)
- 인증 실패가 반복되면 연결이 끊겨 **토큰 무차별 대입**을 막습니다.
- 셀폰 자막(`/m`)과 송출(`/`)은 **읽기 전용**이라 토큰 없이 봐도 됩니다.
- 더 엄격히 하려면 `WS_HOST=127.0.0.1` (이 PC 에서만, 폰 자막은 불가).

> 모니터링: `GET /health` 가 백엔드·언어·장치·접속수를 JSON 으로 반환합니다.
> ⚠️ 이 서버를 인터넷(ngrok 등)에 그대로 노출하지 마세요. LAN 사용을 전제로 설계돼 있습니다.

## 설교 원고로 전사 정확도 높이기 (권장)

**전사가 틀리면 번역도 틀립니다**(예: "히브리어"를 "티브리어"로 들으면 "Hebrew"가
아닌 엉뚱한 번역). 설교 원고를 미리 넣으면 이를 자동으로 바로잡습니다.

- 운영자 화면 **"설교 원고"** 칸에 원고를 **붙여넣고 "원고 적용"**
- 원고에서 **3음절+ 고유명사·용어를 자동 추출**해, 전사 결과를 **발음 유사도로 교정**
  (예: "티브리어"→"히브리어", "아브라암"→"아브라함")
- 한 음절 차이만 보수적으로 교정하므로 정상 전사를 훼손하지 않습니다
- 로컬 모드에선 **번역 전에** 교정되어 번역까지 정확해집니다
- 원고는 `data/sermons/` 에 저장되어 재시작 후에도 유지(git 제외)

> STT 에 단어 목록을 직접 주입하면 그 단어를 토해내는 환각이 있어, 안전하게
> **전사 후 후처리 교정** 방식을 씁니다. 특정 오인식이 반복되면 용어집 `fix:` 규칙으로
> 확실히 고정할 수도 있습니다.
